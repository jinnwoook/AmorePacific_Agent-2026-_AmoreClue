...
export default function ReviewKeywordsPanel({ keywords, itemName, isCombination = false }: ReviewKeywordsPanelProps) {
  ...
  // 리뷰 데이터 생성 함수
  const generateReviewDetail = (keyword: string, isPositive: boolean) => {
    const platforms: Array<'Instagram' | 'TikTok' | 'YouTube' | 'Amazon' | 'Shopee' | 'Cosme' | 'Naver' | 'Kakao'> = 
      ['Instagram', 'TikTok', 'YouTube', 'Amazon', 'Naver', 'Kakao'];
    const platform = platforms[Math.floor(Math.random() * platforms.length)];
    
    const positiveReviews: Record<string, string> = {
      '빠른효과': '사용한 지 일주일 만에 확실한 차이를 느꼈어요! 피부톤이 밝아지고 잡티가 많이 줄었습니다. 정말 만족합니다.',
      '체감 개선': '피부가 훨씬 부드러워지고 탄력이 좋아진 게 느껴져요. 주변 사람들도 피부가 좋아졌다고 말해줍니다.',
      '신뢰할수 있음': '성분이 안전하고 검증된 브랜드라 믿고 사용할 수 있어요. 알레르기 반응도 전혀 없었습니다.',
      '가성비': '이 가격에 이 정도 품질이면 정말 가성비 최고예요. 계속 재구매할 예정입니다.',
      '만족': '전반적으로 매우 만족스러운 제품입니다. 사용감도 좋고 효과도 확실해요.',
      '기대이상': '기대했던 것보다 훨씬 좋은 결과가 나왔어요! 정말 놀랐습니다.',
      '지속적 효과': '사용을 중단해도 효과가 지속되는 게 신기해요. 정말 만족합니다.',
    };

    const negativeReviews: Record<string, string> = {
      '효과 미미': '사용한 지 한 달이 넘었는데 별다른 효과를 느끼지 못했어요. 기대했던 것과 달라서 아쉽습니다.',
      '가격 부담': '가격이 좀 부담스러운 편이에요. 효과는 있지만 가성비는 그리 좋지 않은 것 같습니다.',
      '사용법 복잡': '사용법이 복잡해서 매일 사용하기가 번거로워요. 좀 더 간단했으면 좋겠습니다.',
      '자극': '사용 후 약간의 따가움을 느꼈어요. 민감한 피부에는 부적합할 수 있습니다.',
      '효과 없음': '여러 달 사용했지만 전혀 효과를 느끼지 못했어요. 돈이 아까웠습니다.',
    };

    const reviews = isPositive ? positiveReviews : negativeReviews;
    const review = reviews[keyword] || (isPositive ? '정말 만족스러운 제품입니다!' : '개선이 필요한 부분이 있습니다.');

    const additionalInfos: Record<string, string> = {
      '빠른효과': '이 키워드는 제품 사용 후 빠른 시간 내에 효과를 체감한 소비자들의 리뷰를 나타냅니다. 주로 1-2주 내에 눈에 띄는 변화를 경험한 사용자들이 남긴 긍정적 피드백입니다.',
      '체감 개선': '실제 피부 상태의 개선을 체감한 소비자들의 리뷰입니다. 피부톤, 탄력, 수분 등 다양한 측면에서의 개선을 경험한 사용자들의 긍정적 평가를 반영합니다.',
      '신뢰할수 있음': '브랜드 신뢰도와 제품 안전성에 대한 긍정적 평가입니다. 성분 안전성, 검증된 브랜드, 부작용 없음 등의 요소를 강조하는 리뷰입니다.',
      '가성비': '가격 대비 효과에 대한 만족도를 나타내는 키워드입니다. 합리적인 가격에 우수한 품질을 제공한다는 평가를 받는 제품에 자주 나타납니다.',
      '만족': '전반적인 제품 만족도를 나타내는 포괄적 키워드입니다. 효과, 사용감, 가격 등 모든 측면에서 만족스러운 평가를 받은 경우입니다.',
      '기대이상': '기대했던 것보다 훨씬 좋은 결과를 얻은 소비자들의 리뷰입니다. 예상을 뛰어넘는 효과에 대한 놀라움과 만족을 표현합니다.',
      '지속적 효과': '사용 중단 후에도 효과가 지속되는 것을 경험한 소비자들의 리뷰입니다. 제품의 장기적 효과에 대한 긍정적 평가입니다.',
      '효과 미미': '제품 사용 후 기대했던 효과를 충분히 느끼지 못한 소비자들의 리뷰입니다. 개인차나 사용 기간 등이 영향을 미칠 수 있습니다.',
      '가격 부담': '제품 가격이 부담스럽다고 느끼는 소비자들의 의견입니다. 효과는 있으나 가성비 측면에서 아쉬움을 표현합니다.',
      '사용법 복잡': '제품 사용법이 복잡하거나 번거롭다고 느끼는 소비자들의 피드백입니다. 사용 편의성 개선이 필요한 부분입니다.',
      '자극': '제품 사용 후 피부 자극을 경험한 소비자들의 리뷰입니다. 민감한 피부 타입의 사용자들에게 주의가 필요할 수 있습니다.',
      '효과 없음': '제품 사용 후 전혀 효과를 느끼지 못한 소비자들의 부정적 평가입니다. 개인차나 제품 적합성 문제일 수 있습니다.',
    };
    ...
    return {
      ...
      additionalInfo: additionalInfos[keyword] || '이 키워드에 대한 상세 정보입니다.',
      isPositive,
    };
  };

  const handleBarClick = (keyword: string, isPositive: boolean) => {
    const reviewDetail = generateReviewDetail(keyword, isPositive);
          ...
          <div className="bg-emerald-50/80 border border-emerald-200/80 rounded-lg p-4">
            <ResponsiveContainer width="100%" height={Math.min(300, positiveData.length * 30)}>
              <BarChart 
                data={positiveData} 
                layout="vertical" 
                margin={{ top: 5, right: 10, bottom: 5, left: 80 }}
                onClick={(data: any) => {
